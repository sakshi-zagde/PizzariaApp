...
const BuildUrPizza = ({
 tname,price,image
  ...
  useEffect(() => {
    setloading(true);
    axios
      .get("http://localhost:7000/Pizzas")
      .then((response) => {
  ...
  return (
    <>
      <Navigation />
        ...
        {PizzasData.map((build)=>(
            ...
            <table border="1" style={{margin:'auto',width:'50%'}}>
                <tr >
                    ...
                    <td >{build.tname}{" "}{"₹."}{build.price}{".00"}</td>
                    <td className='text-danger m-2'><input type="checkbox"/>Add</td>
                </tr>
                {/* <script>
         {function myFunction() {
       ...
       <p className='text-center mt-3 h3' >Total Cost : 80 &nbsp;
             <button type="button" class="btn btn-warning ">Build Ur Pizza</button>
             </p>
